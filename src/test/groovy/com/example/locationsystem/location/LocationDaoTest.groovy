...
class LocationDaoTest extends Specification {
    ...
    def "should find all added locations"() {
        ...
        locations.size() == 2
        locations[0].name == 'name1'
        locations[1].name == 'name2'
    ...
    def "should find all locations with access"() {
        ...
        jdbcTemplate.execute("INSERT INTO users(id,name,password,username) VALUES(2,'name2','pass2','user2')")
    ...
    def "should save location"() {
        ...
        and:
        def savedLocation = jdbcTemplate.queryForObject("SELECT * FROM locations WHERE name = ?", BeanPropertyRowMapper.newInstance(Location.class),location.name)
        savedLocation != null
        savedLocation.name == 'title1'
