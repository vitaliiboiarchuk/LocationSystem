...
class UserDaoTest extends Specification {
    ...
    def "should save user"() {
        ...
        and:
        def savedUser = jdbcTemplate.queryForObject("SELECT * FROM users WHERE username = ?", BeanPropertyRowMapper.newInstance(User.class), user.username)
        savedUser != null
    ...
    def "should find users to share"() {
        given:
        jdbcTemplate.execute("INSERT INTO users(id,name,username,password) VALUES(2,'name2','user2',SHA2('pass2',256))")

        when:
        CompletableFuture<List<User>> futureResult = userDao.findUsersToShare(1L)
        ...
        cleanup:
        jdbcTemplate.execute("DELETE FROM users WHERE username = 'user2'")
    }
